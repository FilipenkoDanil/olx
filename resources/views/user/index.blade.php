            ...
            <div class="col-lg-3 col-xl-3">
                <div class="card-box text-center">
                    ...
                    <div class="text-left mt-3">
                        ...
                        <p class="text-muted text-center font-13"><a href="{{ route('user.ads', $user) }}"><strong>Все
                                    объявления</strong></a></p>
                    </div>
                </div> <!-- end card-box -->
            </div> <!-- end col-->
                    ...
                    <div class="toch-table">
                        @forelse($user->reviews as $review)
                            <table class="table table-striped table-bordered">
                                ...
                                <tr>
                                    <td><strong><a
                                                href="{{ route('user.profile', $review->writer) }}">{{ $review->writer->name }}</a></strong>
                                    </td>
                                    <td class="text-right">
                                        <div class="btn-block cart-put form-inline change">
                                            <strong>{{ $review->created_at->format('d/m/Y') }}</strong>
                                            @if(Auth::id() == $review->writer->id)
                                                <form class="review-form-inline"
                                                      id="delete_comment_{{$review->id}}"
                                                      action="{{route('user.deletereview', $review)}}" method="POST">
                                                    <a href="javascript:{}"
                                                       onclick="document.getElementById('delete_comment_{{$review->id}}').submit();">x</a>
                                                    @csrf
                                                </form>
                                            @endif
                                        </div>
                            ...
                            <form action="{{ route('user.addreview', $user) }}" method="POST"
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        ...
                                        @if(count($user->reviews->where('writer_id', Auth::id())->where('rating', '>', '0')) == 0)
                                            <div class="row">
                                                <div class="col-md-5">
                                                    <div
